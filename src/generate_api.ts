...
const isArrayOfEnum = (schema: ReferenceObject | ArraySchemaObject | NonArraySchemaObject | undefined): schema is ArraySchemaObject => {
    // @ts-ignore
    return (schema as ArraySchemaObject).items !== undefined && (schema as ArraySchemaObject).items.enum !== undefined;
};
...
const resolveType = (propertyName: string, newEnums: any[], definition?: ReferenceObject | SchemaObject): string => {
    ...
    if (propertyName === "permissionTypes") {
        console.log(`Processing ${propertyName}`, definition);
    }
    // @ts-ignore
    ...
    } else if (isArrayOfEnum(definition)) {
        console.log(`Nasel jsem pole enumu...${propertyName}`)
        // @ts-ignore
        newEnums.push({enumName: enumName, values: definition.items.enum});
        return `${enumName}[]`;
